...
> Now that we've found the best log-loss for both the classification tree and the random forest tree, we can work on our final model selection. We're going to compare both models on the test set and declare which one is better.
